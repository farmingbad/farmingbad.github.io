...
async function printBurnedMeth() {
    updateStatus('fetched burned meth...');
    const burnedMeth = await window.contract.methods.balanceOf(burnAddress).call();
    const burned = truncateGwei(burnedMeth);
    const burnedValue = Math.trunc(burned * methPrice);
    updateStatus(`burned Meth: ${burned} $METH / ${burnedValue} USD`);
}
